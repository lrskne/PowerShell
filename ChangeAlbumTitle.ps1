...
{
  'start script'
  $lessons = 'C:\Users\Oceana\Music\SpanishMomDay01'
  $forceDiskNumber = 1

   New-Title $lessons $forceDiskNumber  
  'end script'
  ...
  function New-Title
  {
    ...
    #Get all the files
    'fix later - if path has no children a recurse throws an error'
    #$MusicFiles=Get-ChildItem -recurse -Path $Path -File
    ...
    foreach ($File in $MusicFiles)
    {
      ...
      $MediaInfo=Get-MediaInfo $File.FullName
        ...
        $NewAlbumFriend = ($MediaInfo.Title -split ':', 2)[1]
        
  
        $DiscNumber = $forceDisknumber

        
        if ($DiscNumber.Length -eq 1) {$DiscNumber='0'+$DiscNumber}

        $NewAlbum = "MoM"  + $DiscNumber
       ...
       $MediaInfo.Tag.Artists = "TimothyMoser"
  
       $MediaInfo.Tag.Title =($DiscNumber.ToString()+'-'+$TrackChar+' '+$NewAlbum + $NewAlbumFriend)
   
      
      'Show new file info and new file name'
       $MediaInfo.Tag.Album
       $MediaInfo.Tag.Title
       $MediaInfo.Tag.Track
      ...
      $MediaInfo.Save()
       ...
       Rename-Item $File.FullName ($File.DirectoryName + '\' + $DiscNumber + '-' + $TrackChar + ' ' + $NewAlbum +$NewAlbumFriend)
       $TrackNumber++
    }
  }
  . Invoke-Main
